...
CLASS_ID = 0  # air_pollutant

def detect_smoke_haze(image_path):
...
def create_yolo_labels():
    for image_dir in IMAGE_DIRS:
        image_folder = DATASET_PATH / "images" / image_dir
        label_folder = DATASET_PATH / "labels" / image_dir
        ...
        for image_file in os.listdir(image_folder):
            if image_file.lower().endswith((".jpg", ".jpeg", ".png")):
                ...
                bbox = detect_smoke_haze(image_path)
                if bbox is None:
                    print(f"Skipping {image_file} (No smoke/haze detected)")
                    continue
